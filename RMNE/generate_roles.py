...
from .motif_count import MotifCounterMachine ##changed
from .weisfeiler_lehman_labeling import WeisfeilerLehmanMachine ##changed
import os
...
def structual_roles(args,graphs):
    ...
    for i in range(len(graphs)):
        for role in all_roles_set-all_roles[i]:
            graphs_roles[i][role] = []
    #print(len(structure_feature_list), len(graphs_roles))
...
def add_one_node(graphs_roles,id, roleOFnode,node):
    #print(f"id:{id}", f"role of node: {roleOFnode}")
